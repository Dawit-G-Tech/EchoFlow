...
export const addFile = action({
  ...
  handler: async (ctx, args) => {
    ...
    const { entryId, created } = await rag.add(ctx, {
      // ! Super Imp : What search space to add this to. You can not search accross namespaces
      // ! If not added it consider global
      namespace: orgId,
      text,
      key: filename,
      metadata: {
    ...
    if (!created) {
      console.debug("Entry already exists, skipping insert");
...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    const files = await Promise.all(
      results.page.map((entry) => convertEntryToPublishFile(ctx, entry))
    );
